<template>
  <li :id="id" :class="['item', { dragging: isDragging }]" draggable="true" @dragover="handleDragOver"
    @drop="handleDrop" @dragstart="handleDragStart" @dragend="handleDragEnd">
    <SvgIcon class="item__grab" name="drag" width="10" height="13" />
    <label class="label">
      <input v-model="isCheck" class="label__input" type="checkbox" :checked="checked" :aria-checked="checked"
        aria-labelledby="task-label-{{ id }}" />
      <span v-if="!isEditing" class="label__name">{{ content }}</span>
      <input v-else v-model.trim="newContent" class="label__name-edit" @keyup.enter="saveEdit" />
    </label>
    <div class="item__actions">
      <SvgIcon class="item__edit" name="pencil" width="16" height="16" @click="editHandler" />
      <SvgIcon class="item__delete" name="bin" width="16" height="16" @click="removeTodo(id)" />
    </div>
  </li>
</template>
...
const saveEdit = () => {
  if (newContent.value !== content && newContent.value.length) {
